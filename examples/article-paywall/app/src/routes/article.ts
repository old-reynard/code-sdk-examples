...
router.get('/preview/:item', async (req: Request, res: Response) => {
    ...
    const html = await articles.getArticleContent(item.id, 'preview');
    if (!html) {
        return res.status(404).send('Not found');
    }
...
router.get('/full/:intent', async (req: Request, res: Response) => {
    ...
    const html = await articles.getArticleContent(item.id, 'full');
    if (!html) { return res.status(404).send('Not found'); }

    await htmlResponse(res, 'article-purchased-page', { item, content: { html } });
});

router.get('/paid/:intent', async (req: Request, res: Response) => {
    const intent = req.params.intent;
    const payment = await handleIntentValidation(intent);
    ...
    const html = await articles.getArticleContent(item.id, 'paid');
    if (!html) { return res.status(404).send('Not found'); }
